    ...
    , m_allocations_per_page(allocations_per_page)
{
    ...
    assert(allocation_size >= sizeof(void*) && "Allocation size must be not less than 8 bytes.");
    assert(allocations_per_page > 0 && "At least one allocation per page is required.");
...
void* PagedPoolMemoryResource::allocate(size_t size, size_t alignment) {
    assert(size <= m_allocation_size && "Invalid size.");
    assert(m_allocation_size >= alignment && m_allocation_size % alignment == 0 && "Invalid alignment.");

    if (m_data_head == nullptr) {
        allocate_new_page(m_page_head);
        assert(m_data_head != nullptr);
    }
...
void PagedPoolMemoryResource::deallocate(void* memory) {
    if (memory != nullptr) {
#ifndef NDEBUG
        assert(reinterpret_cast<size_t>(memory) % m_allocation_size == 0 && "Invalid alignment.");
        ...
        assert(valid_range && "Memory out of range.");
#endif
